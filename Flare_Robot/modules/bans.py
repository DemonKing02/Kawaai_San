...
def ban(update: Update, context: CallbackContext) -> str:
    ...
    if is_user_ban_protected(chat, user_id, member) and user not in DEV_USERS:
        if user_id == OWNER_ID:
            ...
            message.reply_text(
                "Fighting this Knight here will put normal user lives at risk."
            )
        elif user_id in DEMONS:
            message.reply_text(
                "I can't ban my Magic Hero, He can put a Glabal spell on you!."
            )
        elif user_id in TIGERS:
            message.reply_text(
                "Bring an order from Upper rank Heros to fight a This Hero."
            )
        elif user_id in WOLVES:
            message.reply_text("Demi-human abilities make them ban immune!")
        else:
